...
$(document).ready(function () {
    ...
    $('input[type=file]').change(function() {
        ...
        $(this).simpleUpload(top.basePath + "/album-foto/index/upload", {

            allowedExts: ["avi", "mp4", "flv", "wmv"],
            allowedTypes: ["video/mp4"],
            maxFileSize: 2500000000, //25MB in bytes 
            ...
            start: function(file){
                ...
                $('#filename').html('Arquivo: <a href="' + top.basePath + '/storage/album-fotos/' + top.idCliente + '/' + file.name + '" target="_blank">' + file.name + '</a>' + 
                        ...
                        '</a>' + 
                        '&nbsp;&nbsp;&nbsp;' + 
                        '<a href="#" onClick="playerAlbum(\'' + file.name + '\');">' + 
                        '<i class="glyphicon glyphicon-play"></i>Visualizar' + 
                        '</a>');
                $('#progress').html("");
                $('#progressBar').width(0);
                $("#dsAlbum").val(file.name);
            },
