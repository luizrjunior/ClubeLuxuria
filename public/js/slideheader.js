...
(function($) {
    ...
    function chocoRun(slider, kids, settings, nudge){
      ...
      if(settings.effect == 'slide' || vars.randAnim == 'slide'){
        ...
        $('.choco-slice', slider).each(function(){
          ...
          if(i == settings.numberStrips-1){
            setTimeout(function(){
              slice.animate({ width:origWidth, opacity:'1.0' }, 500, '', function(){ slider.trigger('choco:animFinished'); });
            }, (100 + timeBuff));
          } else {
            setTimeout(function(){
              slice.animate({ width:origWidth, opacity:'1.0' }, 500);
            }, (100 + timeBuff));
          }
          timeBuff += 50;
          i++;
        ...
        $('.choco-slice', slider).each(function(){
          $(this).css('height','100%');
          if(i == settings.numberStrips-1){
            $(this).animate({ opacity:'1.0' }, (500*2), '', function(){ slider.trigger('choco:animFinished'); });
          } else {
            $(this).animate({ opacity:'1.0' }, (500*2));
          }
          i++;
        });
      }
  ...
  $.fn.chocoslider.defaults = {
    ...
    autoPause:false, //(para detener el slider luego de hacer click) 
    speedStrip:'slow', //(Velocidad entre las tiras)	 
    effect:'random', //(Hay tres tipos: effect1, effect2, random)
    numberStrips:0, //(cantidad de tiras a mostrar)
    sliderDelay:3000, //(tiempo de retraso entre la transición de imágenes)
    transparencytitle:0.6, //(Transparencia del título)
    ...
    controlNavigation:false //(Si se quita solo se mostrarán las imágenes)
  };
